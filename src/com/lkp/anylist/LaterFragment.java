...
public class LaterFragment extends Fragment {
    ...
    public static LaterFragment  newInstance(int sectionNumber) {
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    long DBtime=Long.parseLong(timeData.substring(0,10).replace("/",""));


                    Calendar calendar=Calendar.getInstance();
                    calendar.add(Calendar.DATE,1);
                    Date date = calendar.getTime();
                    long DateTime=Long.parseLong(df.format(date));
